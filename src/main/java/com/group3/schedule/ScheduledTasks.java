...
public class ScheduledTasks implements CommandLineRunner {
	...
	// ScheduledTasks will begin a thread and run after the Driver finishes
	// initialization
	@Override
	...
	// documentation for cron format:
	// https://docs.spring.io/spring-framework/docs/current/reference/html/integration.html#scheduling-cron-expression

	/*
	 * Daily Roll Resetter scheduled for 00:00:00 daily: get list of all gamers, for
	 * each gamer in list, set daily rolls to 10, save updated gamer
	 */
	...
	/*
	 * Check Ban Reseter checks for ban lifts every minute, pull today's date for
	 * reference, get all gamers that are banned, collect them to a list, for each
	 * gamer in the list pull their list of 'ban lift dates,' start a flag for the
	 * user still being banned, check every date in the gamer's list of ban lift
	 * dates, if a ban date is found that falls after today the user is still
	 * banned: stop checking; if the gamer is not still banned, set the gamer role
	 * to Gamer, save the updated Gamer info
	 */
		...
		return gamerService.getGamersByRole(Gamer.Role.BANNED).flatMap(gamer -> {
			...
			for (Date banDate : gamer.getBanDates()) {
				...
				if (!stillBanned) {
					...
					emailService.sendEmail(gamer.getEmail(), "Ban Lifted", "Your ban in GachaGame has lifted. "
	...
	/*
	 * Daily Login Bonus Resetter every day at server midnight collect all gamers to
	 * a list; for every gamer in the list reset their login bonus flag, save the
	 * change;
	 */
	...
	/*
	 * Check Event Start Trigger every 10 seconds check for events that are starting
	 * and need initialized grab the current timestamp, get all events, put in a
	 * list for each event if it's not listed as ongoing, and current time is after
	 * its start, and current time is before its end: mark it as an ongoing event &
	 * get the type of event; if the type is doublestrings set the global variable
	 * stringmod to 2x; if the type is rollmod set the global variable rollmod to
	...
	/*
	 * Check Event End Trigger every 10 seconds check for events that are ending and
	 * need teardown, grab the current timestamp, get all events, put in a list, for
	 * each event: if it is listed as ongoing, and the current time is after its
	 * end, mark it as NOT an ongoing event, get the type of event, if the type is
	 * doublestrings set the global variable stringmod to 1x; if the type is rollmod
	 * set the global variable rollmod to 1x save the changes
	 */
	...
	/*
	 * Check Ongoing Events in case of server going down mid event, this method
	 * checks the database for any ongoing events and updates any associated global
	 * modifiers
	 */
	...
	/*
	 * Check Encounter Completion checks the reward token repository for tokens that
	 * are marked as not completed, if the current time is after the end time: the
	 * encounter token is marked complete, the list of collectibles on the encounter
	 * is retrieved and marked as not being on an encounter, the collectible changes
	 * are saved, the reward is distributed to the gamer (which is saved in the
	 * distribution method), and the updated token is finally saved.
	 */

	@Scheduled(cron = "*/10 * * * * *")
	public List<Collectible> checkEncounterCompletion() {
		Date current = Date.from(Instant.now());
		return encounterService.viewCompletedTokens(false).filter(token -> current.after(token.getEndTime()))
				.flatMap(token -> {
