...
public class CollectibleServiceImpl implements CollectibleService {
	...
	public Mono<Collectible> createCollectible(Collectible c) {
		...
		return repo.findByGamerId(c.getGamerId()).defaultIfEmpty(emptyCollectible).collectList().flatMap(owned -> {
	...
	public Mono<ResponseEntity<Object>> upgradeCollectible(UUID collectibleId) {
		return repo.findById(collectibleId)
				.flatMap(collectible -> gamerRepo.findById(collectible.getGamerId()).flatMap(gamer -> {
					int upgradeCost = collectible.getCurrentStat() * 1000;
					if (gamer.getStrings() < upgradeCost) {
						return Mono.just(ResponseEntity.badRequest().body("Insufficient strings to upgrade."));
					} else {
						...
						return gamerRepo.save(gamer).flatMap(gg -> {
							...
							return updateCollectible(collectible).thenReturn(ResponseEntity.ok(collectible));
						});
					}
				}));
	}
	...
	public Mono<ResponseEntity<Object>> removeCollectible(UUID collectibleId, UUID gamerId) {
		return repo.findById(collectibleId).flatMap(collectible -> {
			if (!collectible.getGamerId().equals(gamerId)) {
				return Mono
						.just(ResponseEntity.badRequest().body("Collectible does not belong to the specified gamer."));
			} else {
	...
	public Mono<Collectible> collectibleFusion(List<UUID> collectibleIds) {
		...
		List<Collectible> collectibles = getAllCollectibles()
				.filter(collectible -> collectibleIds.contains(collectible.getId())).collectList()
				.defaultIfEmpty(defaultEmpty).block();
		// null check
		...
		nextStageBase = typeServ.getCollectibleType(type)
				.flatMap(currentType -> typeServ.getCollectibleType(currentType.getNextStage()).flatMap(nextType -> {
					if (nextType != null) {
						return Mono.just(nextType);
					} else {
						return Mono.just(currentType);
					}
