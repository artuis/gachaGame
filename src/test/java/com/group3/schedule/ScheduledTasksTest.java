...
class ScheduledTasksTest {
	...
	void checkEncounterCompletionReturnsCollectibles() {
		...
		Mockito.when(es.distributeReward(before.getReward(), before.getGamerID())).thenReturn(Mono.just(gamer1));
		Mockito.when(es.updateRewardToken(Mockito.any())).thenReturn(Mono.just(after));
